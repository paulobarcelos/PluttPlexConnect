...
import Settings
from PlexConnectServerFinder import Finder # finds a PlexConnect server in the network
        ...
        while True:
            ...
            try:
                ...
                if domain in intercept:
                    dprint(__name__, 1, "***intercept request and forward to PlexConnect")
                    plexConnect_IP = server_finder.get_ip()
                    DNS_forward.sendto(data, (plexConnect_IP, 53))
                    paket, addr_master = DNS_forward.recvfrom(1024)
                    # todo: double check: ID has to be the same!
                    # todo: spawn thread to wait in parallel
                    dprint(__name__, 1, "-> DNS response from "+plexConnect_IP)
